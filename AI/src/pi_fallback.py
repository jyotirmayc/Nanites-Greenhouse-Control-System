...
- Keeps track of last cloud command timestamp
- If cloud silent for CLOUD_TIMEOUT_S, Pi runs local (lightweight) models
...
TOP_CMD_CLOUD = f"greenhouse/{BAY}/cmd"  # incoming cloud commands show up here as well
CLOUD_TIMEOUT_S = cfg.get('fallback', {}).get('cloud_timeout_s', 90)

# Load local (same) models
...
# map decision heuristics (same logic as cloud_controller)
...
while True:
    # if we've seen a cloud cmd recently, do nothing; otherwise act
    if last_cloud_cmd_ts and (now_ts() - last_cloud_cmd_ts) < timedelta(seconds=CLOUD_TIMEOUT_S):
        time.sleep(1.0)
        continue
    # no recent cloud command -> run local models on last telemetry
    if last_tele is None:
        time.sleep(0.5)
        continue
    payload = last_tele
    # build features for irrigation model as in cloud_controller
    ...
    # anomaly detection
    anom_vec = [payload.get('T',0), payload.get('RH',0), payload.get('soil_theta',0), payload.get('PPFD',0), payload.get('CO2',0)]
    is_anom = anom_model.predict([anom_vec])[0] == -1
    if is_anom:
        client.publish(f"greenhouse/{BAY}/alerts", json.dumps({"ts":now_ts().isoformat(),"type":"anomaly","detail":"pi-detected"}))
